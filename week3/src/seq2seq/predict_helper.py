...
def batch_greedy_decode(model, batch_data, vocab, params):
    ...
    inps = torch.tensor(batch_data).to(params['device'])

    # 0. 初始化隐藏层输入, pytorch 中RNN不输入hiddend，则默认初始为0
    # hidden = [torch.zeros((1, batch_size, params['enc_units']))]  # pytorch的hidden格式
    # 1. 构建encoder
    enc_output, enc_hidden = model.encoder(inps)
    # 2. 复制
    dec_hidden = enc_hidden.clone()
    # 3. <START> * BATCH_SIZE
    dec_input = torch.unsqueeze(torch.tensor([vocab.START_DECODING_INDEX] * batch_size), 1).to(params['device'])
    ...
    for t in range(params['max_dec_len']):
        ...
        for index, predicted_id in enumerate(predicted_ids.cpu().numpy()):
            preidicts[index] += vocab.id_to_word(predicted_id) + ' '


        # 上一步结果用于下一步预测
        dec_input = torch.unsqueeze(predicted_ids, 1)
...
def beam_decode(model, batch, vocab, params):
    ...
    def decoder_one_step(enc_output, dec_input, dec_hidden):
        ...
        # 拿到top k个index 和 概率
        top_k_probs, top_k_ids = final_pred.topk(k=params["beam_size"] * 2, dim=-1, largest=True, sorted=True)
    ...
    while steps < params['max_dec_len'] and len(results) < params['beam_size']:
        print(f'steps {steps}/{params["max_dec_len"]}')
        # 获取最新待使用的token
        latest_tokens = [h.latest_token for h in hyps]
        # 替换掉 oov token unknown token
        latest_tokens = torch.tensor([t if t in vocab.id2word else unk_index for t in latest_tokens])

        # 获取所有隐藏层状态
        hiddens = [h.hidden for h in hyps]

        dec_input = torch.unsqueeze(latest_tokens, dim=1).to(params['device'])
        dec_hidden = torch.stack(hiddens, dim=0)
        ...
        for i in range(num_orig_hyps):
            ...
            for j in range(params['beam_size'] * 2):
                ...
                补全代码
                '''
                new_hyp = h.extend(token=top_k_ids[i, j].cpu().tolist(),
