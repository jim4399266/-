...
def scaled_dot_product_attention(q, k, v, mask):
    ...
    scaled_attention_logits = matmul_qk / tf.sqrt(tf.cast(tf.shape(k)[-1], tf.float32))
    # ------------------------------------------------------------------------------
...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    def call(self, v, k, q, mask):
        ...
        q_splited = self.split_heads(q, batch_size) # (batch_size, num_heads, seq_len, depth)
        k_splited = self.split_heads(k, batch_size)
        v_splited = self.split_heads(v, batch_size)

        scaled_attention, attention_weights = scaled_dot_product_attention(q_splited, k_splited, v_splited, mask)
...
def create_masks(inp, tar):
    # 编码器填充遮挡
    enc_padding_mask = create_padding_mask(inp) # (batch_size, 1, 1, sen_len)

    # 在解码器的第二个注意力模块使用。
    # 该填充遮挡用于遮挡编码器的输出。
    dec_padding_mask = create_padding_mask(inp) # (batch_size, 1, 1, sen_len)

    # 在解码器的第一个注意力模块使用。
    # 用于填充（pad）和遮挡（mask）解码器获取到的输入的后续标记（future tokens）。
    look_ahead_mask = create_look_ahead_mask(tf.shape(tar)[1]) # (sen_len, sen_len)
    dec_target_padding_mask = create_padding_mask(tar) # (batch_size, 1, 1, sen_len)
    combined_mask = tf.maximum(dec_target_padding_mask, look_ahead_mask) # (batch_size, 1, sen_len, sen_len)
